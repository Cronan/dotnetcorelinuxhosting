...
PyNet_Args *PyNet_Init(int ext)
{
    ...
    const char* clrFilesPath = "/usr/share/dotnet/shared/Microsoft.NETCore.App/2.0.0/";
    const char* managedAssemblyPath = "./dotnet/bin/Debug/netcoreapp2.0/dotnet.dll";
    ...
    {
        const char* err_exists = "Managed assembly not found";
        pn_args->error = err_exists;
        return pn_args;
    }
    ...
    {
        const char* err_file = "The specified managed assembly is not a file";
        fprintf(stderr, "The specified managed assembly is not a file\n");
        pn_args->error = err_file;
        return pn_args;
    }
    ...
    {
        const char* err_path = "Could not get full path";
        pn_args->error = err_path;
        return pn_args;
    }
    ...
    {
        const char* err_abs = "Failed to convert managed assembly path to absolute path";
        pn_args->error = err_abs;
        return pn_args;
    }
...
double sum(PyNet_Args *pn_args, double a, double b)
{
    typedef double (*sum_ptr)(double, double);
    sum_ptr dele;
    int ret = pn_args->createDelegate(pn_args->domain, pn_args->domainId,"dotnet","dotnet.Program","sum", reinterpret_cast<void **>(&dele));

    if (ret < 0)
    {
        const char* ret_error = "Couldn't create delegate";
        pn_args->error = ret_error;
        return -1;
    }
            ...
            const char** managedAssemblyArgv)
{
    ...
    if (coreclrLib != nullptr)
    {
        ...
        {
            fprintf(stderr, "Function coreclr_createDelegate not found in the libcoreclr.so\n");
        }
            ...
            PyNet_Args* pn_args)
{
    ...
    {
        fprintf(stderr, "Absolute path to libcoreclr.so too long\n");
        const char* err_long = "Absolute path to libcoreclr.so too long";
        pn_args->error = err_long;
        return -1;
    }
    ...
    if (coreclrLib != nullptr)
    {
        coreclr_initialize_ptr initializeCoreCLR = (coreclr_initialize_ptr)dlsym(coreclrLib, "coreclr_initialize");
        coreclr_execute_assembly_ptr executeAssembly = (coreclr_execute_assembly_ptr)dlsym(coreclrLib, "coreclr_execute_assembly");
        pn_args->createDelegate = (coreclr_create_delegate_ptr)dlsym(coreclrLib, "coreclr_create_delegate");
        pn_args->shutdownCoreCLR = (coreclr_shutdown_2_ptr)dlsym(coreclrLib, "coreclr_shutdown_2");

        if (initializeCoreCLR == nullptr)
        {
            fprintf(stderr, "Function coreclr_initialize not found in the libcoreclr.so\n");
        }
        else if (pn_args->createDelegate == nullptr)
        {
            fprintf(stderr, "Function coreclr_createDelegate not found in the libcoreclr.so\n");
        }
        else if (executeAssembly == nullptr)
        ...
        else if (pn_args->shutdownCoreCLR == nullptr)
        {
            fprintf(stderr, "Function coreclr_shutdown_2 not found in the libcoreclr.so\n");
        }
        else
        {
            ...
            {
                fprintf(stderr, "coreclr_initialize failed - status: 0x%08x\n", st);
                const char* err_failed = "coreclr_initialize failed";
                pn_args->error = err_failed;
                exitCode = -1;
            }
            else 
            {
                pn_args->domain = hostHandle;
                pn_args->domainId = domainId;
                exitCode = 0;
            }
        }
    }
...
{
    int latchedExitCode = 0;
    int st = pn_args->shutdownCoreCLR(pn_args->domain, pn_args->domainId, &latchedExitCode);
    if (!SUCCEEDED(st))
    {
        fprintf(stderr, "coreclr_shutdown failed - status: 0x%08x\n", st);
    }
